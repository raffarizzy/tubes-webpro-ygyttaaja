...
**Konteks:** File utama Express.js yang mendaftarkan seluruh middleware, routes, dan handler 404. Juga menangani prefix URL untuk deployment di cPanel serta pengujian koneksi database saat start-up.
...
// Test Database Connection saat start-up
db.getConnection()
    .then((connection) => {
...
**Konteks:** Service layer yang berinteraksi langsung dengan database MySQL. Menggunakan raw SQL dengan JOIN untuk mengambil data produk beserta informasi toko, status verifikasi penjual, dan kategorinya. Seluruh query produk menerapkan filter **soft delete** (`deleted_at IS NULL`). Ditampilkan fungsi `getById`, `create`, dan `delete`.

```javascript
const db = require('../config/db');

// Ambil detail produk by ID (JOIN toko, user, & kategori) — hanya produk aktif
exports.getById = async (id) => {
  const [rows] = await db.query(
    `SELECT p.id, p.nama, p.deskripsi, p.harga, p.diskon, p.stok, p.berat, p.imagePath,
            p.toko_id, t.nama_toko, t.lokasi AS toko_lokasi, t.logo_path AS toko_logo,
            u.is_verified_seller,
            p.category_id, c.judulKategori AS category_nama, p.created_at, p.updated_at
     FROM products p
...
// Buat produk baru (berat default 1000 gram bila tidak diisi)
exports.create = async (data) => {
  ...
  const [result] = await db.query(
    `INSERT INTO products
       (toko_id, category_id, nama, deskripsi, harga, diskon, stok, berat, imagePath, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
    [
...
// Hapus produk (Soft Delete) — bersihkan keranjang & rating, lalu set deleted_at
exports.delete = async (id) => {
  const [rows] = await db.query(
    'SELECT id FROM products WHERE id = ? AND deleted_at IS NULL', [id]
  );
  ...
  await db.query('UPDATE products SET deleted_at = NOW() WHERE id = ?', [id]); // soft delete

  return { message: 'Produk berhasil dihapus' };
};
```

> Catatan teknis: penggunaan **soft delete** (`deleted_at`) menjaga integritas riwayat pesanan — produk yang sudah pernah dibeli tetap tercatat meski "dihapus" dari katalog aktif.
...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0)
      return response.validationError(res, "Field items wajib diisi dan harus berupa array dengan minimal 1 item");
...
**Konteks:** Fungsi `pay()` di Laravel berfungsi sebagai jembatan antara frontend dan payment gateway Duitku. Laravel membuat signature SHA-256, memanggil API Duitku untuk mendapatkan `paymentUrl`, lalu menyinkronkan status & referensi pembayaran ke Node.js API.
...
**Konteks:** Mengelola logika keranjang belanja: mengambil isi keranjang (JOIN ke produk dan toko, hanya produk aktif `deleted_at IS NULL`), menambah item (otomatis membuat keranjang baru jika belum ada, dan menggabungkan jika produk sudah ada), serta memperbarui/menghapus item. Validasi stok dilakukan sebelum setiap penambahan.

```javascript
const db = require('../config/db');

// Ambil isi keranjang user (JOIN produk & toko) — hanya produk aktif
exports.getCartItems = async (userId) => {
  const [rows] = await db.query(`
    SELECT bk.id, bk.keranjang_id, bk.product_id, bk.jumlah, bk.harga,
           p.nama AS product_nama, p.deskripsi AS product_deskripsi,
           p.imagePath AS product_imagePath, p.stok AS product_stok,
           p.harga AS product_harga_current, t.nama_toko, t.lokasi AS toko_lokasi
...
**Konteks:** Mengelola CRUD toko penjual. Fungsi `create` memvalidasi agar satu user hanya bisa memiliki satu toko, lengkap dengan data wilayah (provinsi, kota, kecamatan, kode pos, kode wilayah). Fungsi `delete` menerapkan cascade manual — semua produk toko dihapus terlebih dahulu sebelum toko dihapus dari database.
...
exports.create = async (data) => {
  ...
  const [result] = await db.query(
    `INSERT INTO tokos
       (user_id, nama_toko, deskripsi_toko, lokasi, provinsi, kota, kecamatan, kode_pos, kode_wilayah, logo_path, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
    [data.user_id, data.nama_toko, data.deskripsi_toko || null,
     data.lokasi || null, data.provinsi || null, data.kota || null,
     data.kecamatan || null, data.kode_pos || null, data.kode_wilayah || null, data.logo_path || null]
  );
  return { id: result.insertId, message: 'Toko berhasil dibuat' };
};

// Update toko (verifikasi ownership: id toko + user_id harus cocok)
exports.update = async (tokoId, userId, data) => {
  ...
  const old = rows[0];
  await db.query(
    `UPDATE tokos SET nama_toko=?, deskripsi_toko=?, lokasi=?, provinsi=?, kota=?,
       kecamatan=?, kode_pos=?, kode_wilayah=?, logo_path=?, updated_at=NOW()
     WHERE id=? AND user_id=?`,
    [data.nama_toko ?? old.nama_toko, data.deskripsi_toko ?? old.deskripsi_toko,
     data.lokasi ?? old.lokasi, data.provinsi ?? old.provinsi, data.kota ?? old.kota,
     data.kecamatan ?? old.kecamatan, data.kode_pos ?? old.kode_pos,
     data.kode_wilayah ?? old.kode_wilayah, data.logo_path ?? old.logo_path, tokoId, userId]
  );
...
exports.createAlamat = async (userId, data) => {
  ...
  try {
    ...
    const { alamat, provinsi, kota, kecamatan, kode_pos, kode_wilayah,
    ...
    const [result] = await connection.query(
      ...
      [userId, alamat, provinsi || null, kota || null, kecamatan || null,
       kode_pos || null, kode_wilayah || null, nama_penerima, nomor_penerima, isDefaultValue, now, now]
    );
...
// Hapus alamat — set alamat pertama tersisa sebagai default baru jika yang dihapus adalah default
...
**File:** `routes/web.php`
**Konteks:** Mendefinisikan seluruh route web aplikasi Medcom. Route dikelompokkan berdasarkan akses: public (beranda, detail produk, daftar produk) dan protected dengan middleware `auth` (keranjang, checkout, pesanan, toko, profil, dll.). Ini mencerminkan arsitektur dual-role — satu pengguna bisa mengakses fitur pembeli dan penjual sekaligus.
...
// Muat data produk dari Node.js API (sesuai environment)
...
**Konteks:** Mengorkestrasi seluruh flow checkout di sisi browser: memuat tarif ongkir dari KlikResi via Laravel (`/api/shipping/rates`), menghitung total harga, membuat order ke Node.js API (`/api/orders`), lalu mengirim request pembayaran ke Duitku (`/checkout/pay`) dan mengarahkan pengguna ke payment page.
...
async function loadShippingRates(destinationId) {
    ...
    if (currentCheckoutData.length === 0) throw new Error("Data checkout tidak ditemukan");
...
payButton.addEventListener("click", async () => {
    ...
    try {
        ...
        const paymentResponse = await fetch("/checkout/pay", {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-CSRF-TOKEN": getCsrfToken() },
            body: JSON.stringify({ order_id: orderId, alamat_id: selectedAddress.id, total }),
        });
...
- Di bawah setiap blok kode, sertakan **1 kalimat penutup** yang menjelaskan poin teknis penting dari kode tersebut (misal: penggunaan transaction, soft delete, pola validasi, dll.)
