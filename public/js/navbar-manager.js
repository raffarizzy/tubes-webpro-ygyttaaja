// =====================================================
// UNIFIED NAVBAR SYSTEM - SpareHub (FIXED VERSION)
// Handles: User Auth Display, Cart Count, Active Menu, Logout
// =====================================================

console.log('%c🔧 Navbar Manager Loading...', 'color: #4CAF50; font-weight: bold');
...
class NavbarManager {
  ...
  log(message, data = null) {
    if (this.debugMode) {
      console.log(`%c[NavbarManager] ${message}`, 'color: #2196F3', data || '');
    }
  }

  error(message, error = null) {
    console.error(`%c[NavbarManager ERROR] ${message}`, 'color: #f44336', error || '');
  }
    ...
    try {
      ...
      if (this.currentUser && this.currentUser.nama) {
        ...
        profilSection.innerHTML = `
          ...
          <a href="edit_profil.html" class="user-name-link">
            <span class="user-name">${this.escapeHtml(this.currentUser.nama)}</span>
          </a>
  ...
  updateCartCount() {
    try {
      ...
      if (!cartCountElement) {
        this.log('Cart count element not found (this is OK for pages without cart counter)');
        return;
      }
      ...
      const userCart = this.cartData.filter(item => item.userId === userId);
  ...
  setActiveMenu() {
    try {
      ...
      this.log('Target page for highlighting:', targetPage);

      // Find and activate the correct link
      let activated = false;
      navLinks.forEach(link => {
        const href = link.getAttribute('href');
        this.log(`Checking link: ${href} against ${targetPage}`);
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    console.log('%c📍 DOM Ready - Initializing NavbarManager', 'color: #4CAF50; font-weight: bold');
    navbarManager = new NavbarManager();
    window.navbarManager = navbarManager;
  });
} else {
  console.log('%c📍 DOM Already Ready - Initializing NavbarManager', 'color: #4CAF50; font-weight: bold');
  navbarManager = new NavbarManager();
  window.navbarManager = navbarManager;
}
