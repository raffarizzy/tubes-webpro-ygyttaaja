  ...
  try {
    ...
    for (const item of items) {
      if (!item.product_id || !item.jumlah) {
        return response.validationError(res, 'Setiap item harus memiliki product_id dan jumlah');
      }
...
exports.getOrdersByUserId = async (req, res) => {
  try {
    ...
    return response.error(res, 'Gagal mengambil daftar order', 500, error.message);
  }
};
